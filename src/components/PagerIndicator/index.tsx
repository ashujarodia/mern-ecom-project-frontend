...
type PagerIndicatorProps = React.DetailedHTMLProps<React.ButtonHTMLAttributes<HTMLDivElement>, HTMLDivElement> &
	Partial<{
		activeCss: string;
		inactiveCss: string;
...
const PagerIndicator = ({ className, activeCss, inactiveCss, sliderRef, count, activeIndex, selectedWrapperCss = '', unselectedWrapperCss = '', ...restProps }: PagerIndicatorProps) => {
	...
	return (
		...
		>
			{Array(count)
				.fill(0)
				.map((_, i) => {
					const isActive = activeIndex! >= i * slidesToShow && activeIndex! < (i + 1) * slidesToShow;
					return (
						<div
							key={'indicator' + i}
